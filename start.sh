...
# Проверяем доступность ControlNet
echo "Checking ControlNet availability..."
for i in {1..30}; do
  if curl -s -o /dev/null http://127.0.0.1:7860/controlnet/detect; then
    echo "ControlNet is ready (after $i checks)."
    break
  fi
  echo "Waiting ControlNet… ($i/30)"; sleep 2
done
